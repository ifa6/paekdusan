...
namespace Paekdusan {
    ...
    public:
        static string makeResponse(int statusCode, const string& connection = "close", const string& contentType = "text/html", const string& content = "") {
            string codeDescription;
            if (statusCode == OK) {
                return stringFormat(HTTP_RESPONSE, OK, "OK", connection.c_str(), contentType.c_str(), content.length(), content.c_str());
    ...
    private:
        static string badRequest() {
            ...
            return stringFormat(HTTP_RESPONSE, 400, "BAD REQUEST", "close", "text/html", content.length(), content.c_str());
        }

        static string notFound() {
            string content = "<HTML><TITLE>Not Found</TITLE>\n<BODY><P>NOT FOUND</P>\n</BODY></HTML>\n";
            return stringFormat(HTTP_RESPONSE, 404, "NOT FOUND", "close", "text/html", content.length(), content.c_str());
        }

        static string unimplemented() {
            string content = "<HTML><TITLE>Method Not Implemented</TITLE>\n<BODY><P>Method Not Implemented</P>\n</BODY></HTML>\n";
            return stringFormat(HTTP_RESPONSE, 501, "METHOD NOT IMPLEMENTED", "close", "text/html", content.length(), content.c_str());
        }
    };
}
